'use client';

import { useSearchParams } from 'next/navigation';
import { signIn } from 'next-auth/react';
import { ArrowLeft } from 'lucide-react';
import Link from 'next/link';
import { useActiveCommunity } from '@/hooks/useActiveCommunity';

function GoogleIcon() {
  return (
    <svg className="w-4 h-4" viewBox="0 0 24 24" aria-hidden>
      <path
        fill="currentColor"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="currentColor"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="currentColor"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="currentColor"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}

export default function LoginPage() {
  const searchParams = useSearchParams();
  const { href } = useActiveCommunity();
  const callbackUrl = searchParams.get('callbackUrl') || href('/');

  const handleGoogleSignIn = () => {
    // Dopo OAuth passa da /auth/callback per verificare username/avatar
    signIn('google', {
      callbackUrl: `/auth/callback?callbackUrl=${encodeURIComponent(callbackUrl)}`,
    });
  };

  return (
    <div className="space-y-5 max-w-md">
      <div className="flex items-center gap-3">
        <Link href={href('/')} className="text-gray-400 hover:text-gray-700">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Accedi</h1>
          <p className="text-xs text-gray-500 mt-0.5">Entra nel registro decisionale</p>
        </div>
      </div>

      <div className="reddit-card p-6 space-y-5">
        <p className="text-xs text-gray-600 leading-relaxed">
          Accedi con Google per compilare schede, aprire la Decision Bank e gestire il profilo.
        </p>

        <button
          type="button"
          onClick={handleGoogleSignIn}
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-lg text-xs font-semibold text-gray-800 hover:bg-gray-50 transition-colors"
        >
          <GoogleIcon />
          Accedi con Google
        </button>

        <div className="pt-4 border-t border-gray-100 text-center">
          <p className="text-xs text-gray-500 mb-1">Non hai ancora un account?</p>
          <Link href="/auth/register" className="text-xs font-semibold text-blue-700 hover:underline">
            Registrati
          </Link>
        </div>
      </div>
    </div>
  );
}
